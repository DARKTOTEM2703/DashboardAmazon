document.addEventListener("DOMContentLoaded", function () {
  ...
  // Función para cambiar a la vista de agregar usuarios con animaciones
  btnMostrarAgregar.addEventListener("click", function (e) {
    ...
    if (seccionVer.style.display !== "none") {
      // Ocultar la sección de ver usuarios con animación
    ...
    setTimeout(() => {
      tituloPagina.textContent = "AGREGAR USUARIOS";
  ...
  // Función para cambiar a la vista de ver usuarios con animaciones
  btnMostrarVer.addEventListener("click", function (e) {
    ...
    if (seccionAgregar.style.display !== "none") {
      ...
      setTimeout(() => {
        ...
        // Cargar usuarios (con un pequeño retraso para mostrar la animación)
        setTimeout(() => {
      ...
      // Si ya estaba oculto, solo cargar usuarios
      setTimeout(() => {
        loadingIndicator.style.display = "none";
        cargarUsuarios();
      }, 600);
    ...
    tituloPagina.classList.add("fade-out");
    setTimeout(() => {
      tituloPagina.textContent = "VER USUARIOS";
      tituloPagina.classList.remove("fade-out");
      tituloPagina.classList.add("fade-in");
    }, 300);
  ...
  function cargarUsuarios() {
    const tablaBody = document.getElementById("tabla-datos-usuarios");
